...
#metrics
with open('../results/tables/age_model_report.json', 'rb') as fp:
    model_test_metrics = json.load(fp)

metrics_df = pd.DataFrame(model_test_metrics).round(3).transpose()
...
overall accuracy of `{python} round(model_test_metrics['accuracy'], 2)` and 
an F1 score of `{python} round(model_test_metrics['macro avg']['f1-score'], 2)` on the test data. 
...
in ZIP format and extracted for preprocessing. The dataset contains 2,278 entries and 8 features after cleaning 
and transformations, with the target variable being age_group. Notably, the target variable exhibited an 
imbalance with two classes: Adult (84%) and Senior (16%).
...
Macro Average F1 score of `{python} round(model_test_metrics['macro avg']['f1-score'], 2)` and 
a decent accuracy of `{python} round(model_test_metrics['accuracy'], 2)`. 
